...
## [1.13.52] - 2026-09-25
### Added
- **The tape's two accept clocks are published.** `HistoryTapeRow.accepted_at` and `HistoryTapeRow.received_at` have ridden every observed row of `GET /history/matches/{matchId}` since 2026-09-24 and were documented nowhere, so a customer doing timing research on a tape had no published field to read. `accepted_at` is the instant our arbiter accepted the state, stamped once and never refreshed, which is the clock to difference against: `timestamp` starts equal to it and is then refreshed while the owning source keeps re-asserting the unchanged state, so on most raw rows `timestamp` is a later re-assertion instant (on match 189679, 164 of its 171 observed rows carry a `timestamp` more than a second after `accepted_at`). `received_at` is the instant the state reached our edge before arbitration, so `accepted_at` minus `received_at` is our own arbitration step. Neither figure is a claim about upstream speed and neither is the court's clock. Both are null on every reconstructed row, which never had a clock.
- **The arrival clock's start date, which is the part that bites.** `received_at` is stamped only as a state arrives, so it can never be filled in afterwards. The earliest one we hold is `2026-09-24T02:00:26Z`; on every observed row accepted before that instant it is null while `timestamp` and `accepted_at` are populated. A study that spans the boundary has to read `accepted_at`, which rides the whole observed corpus. Measured on match 189679 (12 September) after a Basic customer reported it: 171 observed rows, `accepted_at` on all 171, `received_at` on none.
- **What `server` guarantees on a tiebreak row, and what it does not.** Tiebreak rows are never settled on either sequence. The server genuinely changes during a breaker, so there is no constant to take a majority of, and the field is the source's own assertion carried through. Where a breaker's rows skip points the rotation cannot be reconstructed from what is served: you have the server of a state, not of a numbered point, and `meta.points.transitions_legal` against `transitions_total` tells you whether that happened. On the set roll-up row carrying `tiebreak_final`, `server` is the value of that committed set-end state and is NOT a preserved copy of the final breaker point's server. Reported by a Basic customer who could not reconcile 21 tiebreak transitions against the standard rotation and was right not to.

### Fixed
- **`Score.accepted_at` said "absent on history-tape rows".** It has been present on them since 2026-09-24, so the description had been wrong for a day, on the one field a timing study is told to use. Corrected, and the tape row now documents its own copy.

## [1.13.51] - 2026-09-25
### Added
- **The `score_withdrawn` frame is documented on the push feed page, where a streaming consumer looks for it.** 1.13.50 put the record and the frame's fields on the score read and in the schema list, and the frame is a STREAM artifact: it arrives on the same channel as the `score` frame it retracts. `GET /ws-token` now carries the frame itself, the reason it exists (the feed publishes every accepted state and runs no trust deference, while the score read runs the legality gate and can serve an older state), what each `kind` asks you to do, and the three reads that recover a judgement you were offline for. No spec surface changed.

## [1.13.50] - 2026-09-25
### Added
- **A withdrawn score state is now recoverable, and `GET /matches/{matchId}/withdrawals` serves the record.** The `verdict` object has told a stream consumer since 2026-09-23 that a state it holds was taken back, but it is recomputed on every read over the newest state, so it returned null again as soon as the next state was accepted. In a live match that is seconds. A consumer that disconnected and missed the `score_withdrawn` frame had no way back to it: the publisher's duplicate ledgers live inside the publishing process, and nothing else recorded the judgement. Every withdrawal is now written down when the judgement is made, before the frame goes out. Three reads use it. `GET /matches/{matchId}/withdrawals` (ULTRA, the tier that receives the frame) returns every refused `sequence` on a match, oldest first, each with `kind`, `reason`, `withdrawn_at`, `published_at`, the `sequence` served in its place, and the frame exactly as the feed sent it. `GET /matches?withdrawn_since=<instant>` lists the match ids with a withdrawal since then, with a count per match, so a consumer coming back after a gap knows which matches to ask about. And `?sequence=N` on `GET /matches/{matchId}/score` makes `verdict` answer for the one sequence you hold, even after later states have landed. Records are kept 90 days from the withdrawal.
- **The `score_withdrawn` frame's own fields are published for the first time.** `type`, `match_id`, `kind`, `superseded_sequence`, `reason`, `safe_to_resume`, `ts` and `published_at`. It carries NO replacement score and no new sequence, which is the question a customer had to ask us: the state to resume from is the one on the next `score` frame, or on a fresh `GET /matches/{matchId}/score`, and the sequence served in the refused state's place is on the withdrawal record rather than on the frame. `ts` is when the judgement was made, `published_at` when the frame was handed to the feed.

### Changed
- **Only a discarded withdrawal is re-asserted by `?sequence=`.** A `deferred` state may since have been corroborated and a `withheld` read described that moment's scan, so neither answers from the record; a verdict the read produces itself always wins, because it describes the state actually being served. `?sequence=` changes `verdict` and nothing else on the object. A non-integer is `400 bad_sequence` rather than an accepted-and-ignored filter, because a silently dropped lookup would read as "not withdrawn". `?withdrawn_since=` likewise takes only `limit` and `offset` beside it: any match filter, `status` or `updated_since` alongside it is `400 bad_withdrawn_since`.

## [1.13.49] - 2026-09-24
### Fixed
- **The tournament catalogue takes `?tier=`, which it had been accepting and ignoring.** `GET /tournaments` publishes each row's `tier` and `tier_source`, and the two match listings have filtered on the same vocabulary since 1.13.28. The catalogue did not: `?tier=atp_500`, `?tier=laver_cup` and an outright invalid value each returned the entire 10,302-row catalogue under a 200, byte-identical to no filter at all, because the level was attached to a page after it had already been selected. It now selects. The one difference from the match listings is the season, and it is stated on the parameter: a catalogue row has no season of its own, so `?tier=` here reads the CURRENT season, the same season the row's own `tier` field reports, while on `/matches` and `/history/matches` it reads each match's own season. A tournament with no row for the current season matches no value. An invalid value is now a 400 `bad_tier` carrying the vocabulary, as it always was elsewhere.

## [1.13.48] - 2026-09-24
### Fixed
- **`tiebreak_final` on the history tape row is documented for the first time, and the page-first rule the reference stated is no longer true.** A PRO customer wired a bot to the closing score and reported that match 195134 served a `tiebreaks` array of `{p1: 2, p2: 7}` while no tape row carried `tiebreak_final`, and that it stayed that way on every later read. Two things were wrong here. The field had never appeared in this reference on `HistoryTapeRow` at all, though it is served on `GET /history/matches/{matchId}` and a customer had been pointed at it in writing; it is now described beside `is_tiebreak`. And the `MatchPoints` text said a page that *starts* on a roll-up row "has no previous row to read and uses the recorded finals alone", which was true when written and is not true now: since 2026-09-24 such a page reads the one row before it, so the value does not depend on where the page was cut. Both endpoints resolve the score the same way and agree with the response's own `tiebreaks` array.
- **The row that closes a tiebreak set carries the score whichever row the set counter moves on.** The tape's set counter lags the games array by a row: the row that took 195134's first set to 6-7 still read `sets` `[0, 0]`, and the counter reached `[0, 1]` on the row after it. The stamp only fired when the counter grew on that same row, so a set whose counter lagged was never stamped, and nothing revisits a tape. The games on the row now decide it, judged by the tournament's own set test so the Next Gen first-to-4 format is read by its own rule. Measured on the live endpoint before the change: of 9 live matches whose `tiebreaks` array stated a final, 2 carried it on no row, both first-set breakers.

## [1.13.47] - 2026-09-24
### Changed
- **A set completion that does not end the match is assigned a `sequence` only once confirmed.** Live since 2026-09-24 and stated here for the first time. A live scoring display can render the set-ending frame ON set point, before the point is decided, and step back when the point is not won; a customer measured exactly that twice. A rise in `sets` that does not end the match, arriving while the state we hold is set point for the side that would gain the set, is now held until the same source keeps presenting it for 10 s or more, a source at least as trusted presents it, or the same source shows play continuing inside the new set. The unconfirmed frame is never stored, so no `sequence` ever carries it and nothing is published as provisional — a `sequence` that carries a set completion is one that met the rule. Until then `GET /matches/{matchId}/score`, the push feed and the WebSocket all keep serving the set-point state; a frame nobody confirms within 120 s is discarded. A set completion that **ends** the match is exempt and follows the completion rules instead. Measured over the 30 days to 2026-09-24: a median 10 s added between the point and the published set-end, and on a feed that does not re-present states the set-end lands with the first point of the next set (90th percentile 133 s). It does not cover a set genuinely won and then corrected on court, which can still be published and rewound — read `corroborated` beside it, and `verdict` says so when a state is taken back.

## [1.13.46] - 2026-09-24
### Added
- **`corroborated` and `corroborated_age_seconds` on the Score: has an independent source vouched for THIS EXACT state?** Raised by a customer who acted on a completed set that was taken back seconds later. `corroborated` is `true` when a source other than the one that wrote the state has since presented us the same state, `false` when none has — including a brand-new state nobody has had a chance to agree with yet — and `null` on the archive fallback of a retention-trimmed completed match, where `age_seconds` and `sources_count` are null too. `corroborated_age_seconds` is the seconds since that vouch, null when there is none. **This is the field to gate a set-end on.** `sources_count` and `changing_sources_count` are window measures over the match and say nothing about the state in front of you, so a set completion that only one source has announced can sit beside a perfectly healthy count — and a set-end is the one state whose correction changes a result rather than a number. It is evidence, not proof: a vouched state can still be corrected, so compare `corroborated_age_seconds` with `age_seconds` and treat a vouch younger than the state it vouches for as the weaker signal it is. When a state is taken back the `sequence` goes down and `verdict` says so. Present wherever `sources_count` is; a yes/no, never a name.
- **`changing_sources_count` on the Score is documented.** Live since 2026-09-24 and absent from this reference. How many distinct sources CHANGED the score in the last 300 seconds, counted on the content clock alone, so a source re-asserting an unchanged state is not counted — the difference from `sources_count`, which a feed stuck re-sending its last state holds at 1 for as long as the echo lasts while this reads 0. The window is wider on purpose: a point lands every 30-60 s and a changeover is about 90 s. A cross-confirmation is agreement, not a change, so this can read below `sources_count` and never above it.
